...
function addSet(id, max, perc, len, targetReps, repsId) {
  ...
  if (typeof setsDone === "number") {
    setsDone = new Array(setsDone).map(function () {
      return targetReps;
    });
  }
...
function deload(overloadWeek) {
  return {
    ...
    reps: Math.round(overloadWeek.reps * 0.7),
    sets: Math.round(overloadWeek.sets * 0.6),
  };
}
      ...
      workout.forEach(function (exercise) {
        ...
        echo(
          formatExercise(
            ...
            levels.reps,
            levels.sets,
            phase + "--wk-" + week + "--day-" + day + "--" + exercise,
            len[exercise]
          )
        );
      });
